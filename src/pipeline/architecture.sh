...
analysis_grep \
    --include='*.cs' \
    -InE \
    'CreateAssetMenu|:[[:space:]]*ScriptableObject' | redact_signal_content \
...
analysis_grep \
    --include='*.cs' \
    -InE \
    ':[[:space:]]*MonoBehaviour' | redact_signal_content \
...
analysis_grep \
    --include='*.cs' \
    -InE \
    '^[[:space:]]*(public|internal|protected|private)?[[:space:]]*interface[[:space:]]+' | redact_signal_content \
...
analysis_grep \
    --include='*.cs' \
    -InE \
    'UnityEditor|CustomEditor|PropertyDrawer|EditorWindow|MenuItem' | redact_signal_content \
...
analysis_grep \
    --include='*.cs' \
    -InE \
    'Singleton|StateMachine|IState|Command|Observer|EventBus|ServiceLocator|DependencyInjection|Factory|Builder|Strategy|ObjectPool|Repository|Mediator|MVC|MVVM|Presenter' | redact_signal_content \
...
analysis_grep \
    --include='*.cs' \
    -InEi \
    'Photon|Mirror|Fusion|Netcode|NetworkBehaviour|NetworkObject|RPC|ClientRpc|ServerRpc|Bolt|FishNet|Steamworks' | redact_signal_content \
...
analysis_grep \
    --include='*.cs' \
    -InEi \
    'HttpClient|UnityWebRequest|REST|GraphQL|Firebase|Analytics|Telemetry|WebSocket|Socket|API|JsonUtility|Newtonsoft|SQLite|LiteDB|Realm' | redact_signal_content \
...
analysis_grep \
    --include='*.cs' \
    -InEi \
    'Profiler|ObjectPool|pooling|Addressables|async|await|Task|JobHandle|BurstCompile|NativeArray|ECS|EntityManager|GC\.Alloc|Resources\.Unload|AssetBundle' | redact_signal_content \
...
analysis_grep \
    --include='*.cs' \
    -InE \
    'TODO|FIXME|HACK|XXX' | redact_signal_content \
