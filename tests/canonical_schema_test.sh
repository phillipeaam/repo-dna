...
python "$ROOT/renderers/validate_json.py" "$TEMP/generic.json" "$ROOT/schemas/generic-analysis-1.2.0.schema.json"
...
PY
if python "$ROOT/renderers/validate_json.py" "$TEMP/invalid.json" "$ROOT/schemas/generic-analysis-1.2.0.schema.json" >/dev/null 2>&1; then
